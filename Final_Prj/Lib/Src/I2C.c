...
static void Config_GPIO_I2C(){
	...
	PB8.GPIO_Pin_Config_t.GPIO_PinSpeed = GPIO_OSPEEDR_HIGH;
	PB8.GPIO_Pin_Config_t.GPIO_PinPuPdControl = GPIO_PUPDR_PU;
	PB8.GPIO_Pin_Config_t.GPIO_PinAltFunMode_High = GPIO_AFRH_AF4;
	GPIO_Init(&PB8);
	...
	PB9.GPIO_Pin_Config_t.GPIO_PinSpeed = GPIO_OSPEEDR_HIGH;
	PB9.GPIO_Pin_Config_t.GPIO_PinPuPdControl = GPIO_PUPDR_PU;
	PB9.GPIO_Pin_Config_t.GPIO_PinAltFunMode_High = GPIO_AFRH_AF4;
	GPIO_Init(&PB9);
}
...
void I2C_Init(I2C_Handle_t *pI2CHandle){
    ...
    temp = 0;
    if(pI2CHandle->I2C_Congif_t.I2C_SCLSpeed <= I2C_SCL_SPEED_SM){	// For normal mode
        // Mode is standard mode
        ccr_value = (RCC_GetPCLK1Value() / (2 * pI2CHandle->I2C_Congif_t.I2C_SCLSpeed));
    ...
    temp = 0;
    if(pI2CHandle->I2C_Congif_t.I2C_SCLSpeed <= I2C_SCL_SPEED_SM){	// Mode is standard mode
    	trise_value = (RCC_GetPCLK1Value() / pI2CHandle->I2C_Congif_t.I2C_SCLSpeed) + 1;
    }
...
void I2C_Address(I2C_RegDef_t *pI2Cx, uint8_t Address){
...
void I2C_MasterSend_Multi_Data(I2C_RegDef_t *pI2Cx, uint8_t *pTxbuffer, uint32_t len){
    // 6. Send the data until Len becomes 0
    while(len > 0){
        while(!(I2C_GetFlagStatus(pI2Cx,I2C_FLAG_TxE))); // Wait till TxE is set
        pI2Cx->DR = (uint8_t)*pTxbuffer;
        pTxbuffer++;
        len--;
    }
