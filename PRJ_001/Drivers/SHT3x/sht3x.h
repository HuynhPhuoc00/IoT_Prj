...
typedef struct{
	void (*i2c_master_send)(I2C_Handle_t *I2C_Handle,
							uint16_t addr,
							uint8_t *pTxbuffer,
							uint32_t len);
	void (*i2c_master_read)(I2C_RegDef_t *pI2Cx,
							uint8_t Address,
							uint8_t *buffer,
							uint8_t size);
	void (*i2c_read_mem)(I2C_RegDef_t *pI2Cx,
							uint16_t DevAddr,
							uint16_t MemAddr,
							uint8_t MemAddrSize,
							uint8_t *pData,
 ...
 */
bool sht3x_check_init(sht3x_handle_t *handle);
 ...
 * @return True on success, false otherwise.
 */
bool sht3x_read_temperature_and_humidity(sht3x_handle_t *handle, int *temperature, int *humidity);
